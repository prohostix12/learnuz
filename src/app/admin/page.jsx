...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    const isAnyModalOpen = uniModalOpen || progModalOpen || !!viewContactModal || testimonialModalOpen;
    if (isAnyModalOpen) {
  ...
  }, [uniModalOpen, progModalOpen, viewContactModal, testimonialModalOpen]);
  ...
  const filteredTestimonials = testimonials.filter(t => 
    ...
    t.university.toLowerCase().includes(testimonialSearch.toLowerCase()) ||
  ...
  const handleTestimonialSubmit = async (e) => {
    ...
    setActionLoading(true);
    try {
      const isEdit = !!editTestimonialTarget;
      const url = isEdit 
        ? `/api/testimonials/${editTestimonialTarget}` 
        : '/api/testimonials';
      const method = isEdit ? 'PUT' : 'POST';
  ...
  const handleEditTestimonial = (testimonial) => {
    setEditTestimonialTarget(testimonial._id);
    setTestimonialForm({
  ...
  const handleTestimonialDelete = async (id) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const err = await res.json();
        alert(err.error || "Failed to delete testimonial");
      }
    } catch (error) {
      console.error(error);
      alert("An error occurred");
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-[#0f172a] flex flex-col font-sans">
      ...
      <main className="flex-grow max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
          ...
          <div className="bg-white p-5 rounded-3xl border border-slate-200/50 shadow-sm flex items-center gap-4">
            ...
            <div>
              <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider block">Testimonials</span>
              <span className="text-2xl font-black text-slate-800 leading-tight">{testimonials.length}</span>
            </div>
          </div>
          ...
          {['overview', 'universities', 'programs', 'leads', 'testimonials'].map((tab) => {
            ...
            return (
              <button
                ...
                className={`flex-grow md:flex-initial text-center px-6 py-3 rounded-xl font-bold text-xs sm:text-sm whitespace-nowrap transition-all duration-200 ${
                  isActive 
              ...
              <div className="bg-white rounded-3xl border border-slate-200/60 shadow-sm p-6 space-y-6">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100">
                  <h3 className="font-extrabold text-lg text-slate-900 flex items-center gap-2">
                    ...
                    <span>All Student Testimonials ({testimonials.length})</span>
                  </h3>
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="pl-9 pr-4 py-2 w-full sm:w-60 rounded-xl border border-slate-200 text-xs font-bold text-slate-800 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 bg-slate-50/50"
                      />
                    </div>
                    {/* Add Testimonial Button */}
                    <button
                      onClick={() => { resetTestimonialForm(); setEditTestimonialTarget(null); setTestimonialModalOpen(true); }}
                      className="group flex items-center gap-1.5 px-4.5 py-2.5 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs font-bold transition-all shadow-sm cursor-pointer"
                    >
                      <Plus className="w-4 h-4" />
                ...
                {filteredTestimonials.length === 0 ? (
                  <div className="text-center py-12">
                    <p className="text-sm text-slate-400">No testimonials found matching your criteria.</p>
                  </div>
                ) : (
                    ...
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="border-b border-slate-100 bg-slate-50/50">
                          <th className="py-3 px-4 text-xs font-bold uppercase tracking-wider text-slate-400">Student Info</th>
                          <th className="py-3 px-4 text-xs font-bold uppercase tracking-wider text-slate-400">University & Course</th>
                          <th className="py-3 px-4 text-xs font-bold uppercase tracking-wider text-slate-400">Quote</th>
                          <th className="py-3 px-4 text-xs font-bold uppercase tracking-wider text-slate-400 text-center">Rating</th>
                          <th className="py-3 px-4 text-xs font-bold uppercase tracking-wider text-slate-400 text-right">Actions</th>
                        </tr>
                      </thead>
                        ...
                        {filteredTestimonials.map((t) => (
                          <tr key={t._id} className="hover:bg-slate-50/40 transition-colors">
                            <td className="py-4 px-4">
                              <div className="flex items-center gap-3">
                                <img
                                  src={t.image || t.avatar || 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&w=100&q=80'}
                                  alt={t.name}
                                  className="w-10 h-10 rounded-full object-cover border border-slate-100"
                                />
                                <div className="min-w-0">
                                  <span className="font-bold text-slate-800 text-xs block">{t.name}</span>
                                  <span className="text-[10px] text-slate-400 font-semibold block">{t.role || 'Student'} {t.country ? `(${t.country})` : ''}</span>
                                </div>
                              </div>
                            </td>
                            <td className="py-4 px-4">
                              <span className="text-xs font-bold text-slate-700 block">{t.university}</span>
                              <span className="text-[10px] text-indigo-500 font-semibold block">{t.course || 'N/A'}</span>
                            </td>
                            <td className="py-4 px-4 max-w-xs md:max-w-md">
                              <p className="text-xs text-slate-500 font-medium leading-relaxed truncate" title={t.quote}>
                                "{t.quote}"
                              </p>
                            </td>
                            ...
                            <td className="py-4 px-4 text-right">
                              <div className="flex items-center justify-end gap-1">
                                <button
                                  ...
                                  className="p-2 rounded-xl text-slate-500 hover:text-blue-600 hover:bg-blue-50/50 transition-colors cursor-pointer"
                                ...
                                <button
                                  ...
                                  className="p-2 rounded-xl text-slate-400 hover:text-red-600 hover:bg-red-50/50 transition-colors cursor-pointer"
      ...
      {uniModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md" data-lenis-prevent>
      ...
      {testimonialModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md" data-lenis-prevent>
          <div className="bg-white w-full max-w-xl rounded-3xl shadow-2xl border border-slate-100 overflow-hidden relative max-h-[90vh] flex flex-col">
            <button
              onClick={() => { setTestimonialModalOpen(false); setEditTestimonialTarget(null); }}
              className="absolute top-5 right-5 w-9 h-9 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-600 flex items-center justify-center transition-colors z-10"
            >
              <X className="w-5 h-5" />
            </button>
            ...
            <div className="p-6 sm:p-8 border-b border-slate-100">
              <h3 className="text-2xl font-black text-slate-900">
                {editTestimonialTarget ? "Edit Testimonial" : "Add New Testimonial"}
              </h3>
              <p className="text-xs text-slate-400 mt-1">Configure student story details, ratings, course program, and quotes.</p>
            </div>

            <form onSubmit={handleTestimonialSubmit} className="flex-grow overflow-y-auto p-6 sm:p-8 space-y-4">
              
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1.5">Student Name</label>
                  <input
                    type="text" required
                ...
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1.5">Role / Title</label>
                  <input
                    type="text" required
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1.5">Rating (1-5 Stars)</label>
                  <select
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1.5">University Partner</label>
                  <input
                    type="text" required
                    value={testimonialForm.university}
                    onChange={(e) => setTestimonialForm({ ...testimonialForm, university: e.target.value })}
                    className="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm focus:outline-none focus:border-blue-500"
                    placeholder="e.g. Oxford Partner Program"
                  />
                </div>
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1.5">Course Program</label>
                  <input
                    type="text"
              ...
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1.5">Student Profile Image URL</label>
                <input
                  type="url"
              ...
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1.5">Testimonial Quote</label>
                <textarea
                  required
                  value={testimonialForm.quote}
                  onChange={(e) => setTestimonialForm({ ...testimonialForm, quote: e.target.value })}
                  rows="4"
                  className="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm focus:outline-none focus:border-blue-500 leading-relaxed"
                  placeholder="Tell us what the student said about their experience..."
                />
              </div>
              ...
              <div className="pt-4 border-t border-slate-100 flex items-center justify-end gap-3">
                <button
                  type="button"
                  onClick={() => { setTestimonialModalOpen(false); setEditTestimonialTarget(null); }}
                  className="px-5 py-3 rounded-xl border border-slate-200 text-slate-500 font-bold text-xs uppercase cursor-pointer"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={actionLoading}
                  className="px-6 py-3 rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs uppercase flex items-center gap-2 cursor-pointer"
                >
                  {actionLoading && <span className="w-3.5 h-3.5 rounded-full border-2 border-slate-500 border-t-white animate-spin"></span>}
